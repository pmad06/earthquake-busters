...
class Trie:
    ...
    def insert(self, word: str, quake_obj: dict):
        ...
        quake_id = len(self.objects)
        self.objects.append({
            "location": quake_obj.get("location", ""),
            "lat": quake_obj.get("lat"),
            "long": quake_obj.get("long"),
            "magnitude": quake_obj.get("magnitude"),
            "url": quake_obj.get("url")
        })
    ...
    def autocomplete(self, prefix : str, limit: int = 0):
        ...
        results = []
        self.collect(node, results, limit)
        return [self.objects[i] for i in results]

    def collect(self, node, out: list, limit: int):
        if node.is_end_of_word:
            if limit > 0:
                remaining = limit - len(out)
                out.extend(node.objects[:remaining])
            else:
